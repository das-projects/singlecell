...
pl.legend(bbox_to_anchor=(1.04, 0.5), loc=2, borderaxespad=0.0)
...
df1["Speedup"] = (
    df["CPU time (min)"][df["toolkit"] == "Cell Ranger (R)"].values
    / df["CPU time (min)"][df["toolkit"] == "Scanpy (Py)"].values
)
df1["memory ratio"] = (
    1
    / df["total memory (GB)"][df["toolkit"] == "Cell Ranger (R)"].values
    * df["total memory (GB)"][df["toolkit"] == "Scanpy (Py)"].values
)
...
g = g.map(pl.plot, "n cells", "Speedup", marker="o").add_legend(
    title=False, handlelength=0.3, frameon=True
)
pl.subplots_adjust(top=0.82, right=0.82)
